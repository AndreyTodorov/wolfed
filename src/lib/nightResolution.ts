...
export function resolveNightActions(
  ...
  actions.forEach((action) => {
    ...
    switch (action.actionType) {
      ...
      case "kill":
        ...
        if (actor.role.id === "werewolf" || actor.role.id === "alpha_wolf") {
          ...
          attacks.get(target.id)!.push({ type: "werewolf", source: actor.name });
        ...
        } else if (actor.role.id === "vampire" || actor.role.id === "nosferatu") {
          if (!attacks.has(target.id)) attacks.set(target.id, []);
          attacks.get(target.id)!.push({ type: "vampire", source: actor.name });
          result.logs.push(`${actor.role.name} attacked ${target.name}`);
  ...
  attacks.forEach((attackList, targetId) => {
    ...
    attackList.forEach((attack) => {
      ...
      if (protection) {
        if (
          (attack.type === "werewolf" && protection.type === "werewolf") ||
          (attack.type === "physical" && protection.type === "physical")
        ) {
          result.logs.push(
            `${target.name} was protected from ${attack.type} attack by ${protection.source}`
          );
          return; // Protected - no death
        }
      }
